...
const std::string SdkRuntime::SDK_BIN_BASE        = "/userdata/slot_b/mower_RGB_sdk_iceoryx/bin/";
const std::string SdkRuntime::SDK_LIB_PATH        = "/userdata/slot_b/mower_RGB_sdk_iceoryx/lib_dl";
...
void SdkRuntime::setupEnvironment()
{
    ...
    if (existingPath && std::string(existingPath).length() > 0)
    {
...
{
    std::string killCommand = "pgrep -f \"" + processName + "\" >/dev/null 2>&1 && pkill -f \"" + processName + "\" >/dev/null 2>&1";
    int ret = std::system(killCommand.c_str());
    (void)ret;
}
